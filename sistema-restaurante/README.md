...
Aplicación web del sistema descrito en [`../docs`](../docs). La base de datos se llama
`ventas_db`: son las ventas del restaurante, y renombrarla obligaría a migrar los datos sin
ganar nada.
...
proveedor. Un restaurante de este tamaño no lleva kardex de insumos, y el plato se hace en la
casa: no hay costo de compra que registrar.
...
|---------|----------|----------|------------|-------|
| `docker-compose.yml` | `restaurante-dev` | desarrollo: el código va montado desde el host | <http://localhost:8200> | `127.0.0.1:13311` |
| `docker-compose.prod.yml` | `restaurante-prod` | el servidor: el código va horneado en la imagen | el dominio del local | no se publica |

Cada uno tiene sus contenedores y su volumen de datos, así que levantar uno no toca la base del
otro. En desarrollo alcanza con `docker compose up -d`.
...
cp sistema-restaurante/.env.docker.example sistema-restaurante/.env.docker
```

Y completa en `sistema-restaurante/.env.docker` la `APP_KEY` (`php -r "echo 'base64:'.base64_encode(random_bytes(32)).PHP_EOL;"`)
...
> La configuración del entorno Docker vive en `sistema-restaurante/.env.docker` (plantilla:
...
Encuentra solo el contenedor de MySQL, en este orden: el de producción
(`restaurante_mysql_prod`) y el de desarrollo (`restaurante_mysql`), y **prefiere el que esté
corriendo**: con los dos instalados, el detenido no le gana al levantado. Lo mismo hacen
...
restaurante no hay que nombrar nada. `RESTAURANTE_MYSQL=...` queda solo para un contenedor con otro
nombre.
...
Hoy no hay ningún parche pendiente: el esquema de `docs/sql/01_schema_mysql.sql` es el punto de
partida y una base creada con él nace completa. Cómo se escribe uno cuando haya que corregir el
esquema de una instalación en marcha está en
[`docs/sql/parches/LEEME.md`](../docs/sql/parches/LEEME.md).
...
siendo `DECIMAL(12,3)`: deja abierta la puerta a vender algo al peso sin migrar la tabla.
...
comanda»**. Lo que no tiene es lector de código de barras: nadie escanea un plato.
...
¿Por qué no registrar la venta suelta, sin pedido? Porque el pedido es lo que
...
Cambiar la hora de corte no renumera nada: los pedidos ya abiertos conservan su número, porque
está impreso en el ticket que el cliente se llevó.
...
Antes la fórmula restaba también las devoluciones, que el restaurante no maneja. Los arqueos cerrados
...
2. **Clave y contraseña propias**, en `sistema-restaurante/.env.docker`:
   ...
   **Cuenta de base de datos de la aplicación.** Pon otra contraseña, distinta, en `DB_APP_PASSWORD`
   del `.env` de la raíz, y en `sistema-restaurante/.env.docker` cambia a `DB_USERNAME=ventas_app` con
...
3. **Compilar los assets** (una vez, y cada vez que cambie el código del frontend). No se
   ...
   ```bash
   cd sistema-restaurante && npm ci && npm run build && cd ..
   ```
...
4. **Levantar, con la vía de producción**:
   ...
   `docker-compose.yml` sirven aquí. Los dos son entornos de **desarrollo**: montan el
...
9. **Cobro por QR**, si el negocio lo va a usar. De fábrica queda en `QR_PASARELA=simulado`:
   ...
   `sistema-restaurante/.env.docker` (nunca en el repositorio):
...
10. **HTTPS**, si el servidor es accesible por internet (no solo en la red del local): un proxy
    ...
    también en `sistema-restaurante/.env.docker`:
...
3. En `sistema-restaurante/.env.docker`, `RESPALDOS_NUBE=drive:`, y `up -d` para que lo lean la
   ...
   ```bash
   docker exec -u www-data restaurante_app_prod php artisan respaldo:crear
   ```

La conexión vive en el volumen `restaurante_rclone` (`storage/app/rclone`), no en la imagen:
...
`sistema-restaurante/.env.docker` `RESPALDOS_COPIA=/respaldos-copia`, y `up -d`. Si la copia falla, el
...
```bash
docker cp restaurante_app_prod:/var/www/html/storage/app/respaldos/<archivo>.sql.gz backups/
```
