...
class MenoresDeCierreTest extends TestCase
{
     ...
     * si no, el script los vería pendientes y los aplicaría a ciegas.
     */
    public function test_el_esquema_registra_los_parches_que_ya_incorpora(): void
    {
        ...
        $parches = glob(base_path('../docs/sql/parches/*.sql'));

        foreach ($parches as $ruta) {
            $archivo = basename($ruta);
            $this->assertStringContainsString("('{$archivo}')", $esquema,
                "{$archivo} falta en el registro de parches de 01_schema_mysql.sql");
        }

        // Y al revés: nada registrado que no exista como archivo.
        preg_match_all("/\('(20\d{2}_[^']+\.sql)'\)/", $esquema, $registrados);
        $this->assertSame(
            collect($parches)->map(fn (string $r) => basename($r))->sort()->values()->all(),
            collect($registrados[1])->unique()->sort()->values()->all(),
        );
    }
