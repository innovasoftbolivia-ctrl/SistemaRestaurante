@php
    ...
    $alertas = AlertasStock::para(auth()->user());
@endphp

{{-- La campana de lo que hay que comprar: solo para quien gestiona el
     inventario, y solo si hay bebidas que llevan stock. --}}
@if ($alertas !== null && ($alertas['total'] > 0 || \App\Models\Producto::conStock()->exists()))
    <div class="relative" x-data="{ abierto: false }" @click.away="abierto = false" @keydown.escape="abierto = false"
        ...
        <button type="button" @click="abierto = !abierto" :aria-expanded="abierto" aria-controls="alertas-stock"
            aria-label="{{ $alertas['total'] === 0 ? 'Stock: nada por comprar' : "Stock: {$alertas['total']} producto(s) por comprar" }}"
        ...
        <div id="alertas-stock" x-show="abierto" x-cloak x-transition.opacity.duration.150ms
            ...
            <div class="border-b border-gray-100 px-4 py-3 dark:border-gray-800">
                <span class="block text-theme-sm font-medium text-gray-800 dark:text-white/90">Por comprar</span>
                <span class="text-theme-xs text-gray-500 dark:text-gray-400">
                    @if ($alertas['total'] === 0)
                        Todo está sobre su stock mínimo.
                    @else
                        {{ $alertas['total'] }} en su mínimo o por debajo{{ $alertas['agotados'] > 0 ? ", {$alertas['agotados']} sin stock" : '' }}.
                    @endif
                </span>
            </div>
                        ...
                        <li>
                            <a href="{{ route('inventario.kardex', $producto) }}"
                                ...
                                <span class="min-w-0">
                                    ...
                                    <span class="text-theme-xs text-gray-500 dark:text-gray-400">{{ $producto->categoria?->nombre }}</span>
                                </span>
                                <span class="shrink-0 text-right">
                                    <span class="block text-theme-sm font-medium tabular-nums {{ $agotado ? 'text-error-600 dark:text-error-400' : 'text-warning-700 dark:text-orange-400' }}">
                                        {{ (float) $producto->stock_actual < 0 ? 'Negativo ('.Config::cantidad($producto->stock_actual).')' : ($agotado ? 'Sin stock' : Config::cantidad($producto->stock_actual).' u.') }}
                                    </span>
            ...
            <a href="{{ route('inventario.index') }}"
                ...
                {{ $alertas['total'] > AlertasStock::MOSTRAR ? "Ver los {$alertas['total']} en el inventario" : 'Ver el inventario' }}
            </a>
        </div>
    </div>
@endif
